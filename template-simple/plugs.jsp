...
<DIV class="widget-area" id="secondary" role="complementary">
	<ASIDE class="widget widget_recent_entries">
	    <form id="searchform" action="${rurl }post/search" method="post">
            <label class="assistive-text" for="s">搜索</label> <input name="key" class="field" id="s" type="text" placeholder="<c:out value="${keywords}" default="请输入关键字" />" value="<c:out value="${keywords}"/>"/>
            <input name="submit" class="submit" id="searchsubmit" type="submit" value="<c:out value="${keywords}" default="请输入关键字" />"/>
        </form>
        <br>
		<c:choose>
		...
		<style type="text/css">
			.taglist a{padding:0px;display:inline-block;white-space:pre-line;}
